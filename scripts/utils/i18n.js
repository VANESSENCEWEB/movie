...
export function applyHeroCopy(heroEl, lang = getLang()) {
  ...
  if (ctaEl) {
    const primaryLabel = ctaEl.querySelector('.btn--shiny .btn__label')
      || ctaEl.querySelector('.btn--shiny span')
      || ctaEl.querySelector('.btn--primary');
    const outlineLabel = ctaEl.querySelector('.btn--outline-hero .btn__label');

    if (primaryLabel) primaryLabel.textContent = copy.ctaPrimary;

    if (outlineLabel) {
      outlineLabel.textContent = copy.ctaWhatsapp;
    } else {
      const outline = ctaEl.querySelector('.btn--outline-hero') || ctaEl.querySelector('.btn--outline');
      if (outline) {
        const icon = outline.querySelector('.btn__icon, svg');
        outline.childNodes.forEach((n) => {
          if (n !== icon) n.remove();
        });
        if (icon) outline.append(` ${copy.ctaWhatsapp}`);
        else outline.textContent = copy.ctaWhatsapp;
      }
    }
  }
